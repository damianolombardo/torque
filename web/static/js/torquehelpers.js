    ...
    if ($('#plot_data').chosen().val() == null) {
        ...
        $.get(varPrm, d => {
            ...
            if ($('#placeholder')[0] == undefined) { //this would only be true the first time we load the chart
                $('#Chart-Container').empty();
                $('#Chart-Container').append($('<div>', { class: 'demo-container' }).append($('<div>', { id: 'placeholder', class: 'demo-placeholder', style: 'height:450px' })));
                doPlot("right");
            }
...
initMapOpenlayers = () => {
    ...
    $('#BaseLayerOpt').val('Openstreetmap.');
    $('#BaseLayerOpt').off('change');
    const updBase = () => { //updates the base layer based on selection
        const selLayer = $('#BaseLayerOpt').find(":selected").val() !== undefined ? $('#BaseLayerOpt').find(":selected").val() : 'Openstreetmap.';
        tileLayer.setUrl(selLyrUrl[selLayer] || selLyrUrl['Openstreetmap.']);
    }
    $('#BaseLayerOpt').on('change', updBase);
    //defines the default base layer
    const tileLayer = new ol.source.XYZ({ url: selLyrUrl['Openstreetmap.'] });
